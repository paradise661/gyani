...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Prices ({{ $prices->total() }})</h5>
            <small class="text-muted float-end">
                <a href="{{ route('price.create') }}" class="btn btn-primary"><i class="fa-solid fa-plus"></i>
                    Create</a>
            </small>
        </div>
            ...
            @if (!$prices->isEmpty())
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($prices as $key => $blog)
                            <tr>
                                <td><strong>{{ $key + $prices->firstItem() }}</strong></td>
                                ...
                                <td>
                                    <a href="{{ route('price.edit', $blog->id) }}"
                                        style="float: left;margin-right: 5px;" class="btn btn-sm btn-primary"><i
                                            class="fa-solid fa-pen-to-square"></i> Edit</a>


                                    <form class="delete-form" action="{{ route('price.destroy', $blog->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger delete_price mr-2" id=""
