...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Offers ({{ $offers->total() }})</h5>
            <small class="text-muted float-end">
                <a href="{{ route('offer.create') }}" class="btn btn-primary"><i class="fa-solid fa-plus"></i>
                    Create</a>
            </small>
        </div>
            ...
            @if (!$offers->isEmpty())
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($offers as $key => $blog)
                            <tr>
                                <td><strong>{{ $key + $offers->firstItem() }}</strong></td>
                                <td class="">
                                ...
                                <td>
                                    <a href="{{ route('offer.edit', $blog->id) }}"
                                        style="float: left;margin-right: 5px;" class="btn btn-sm btn-primary"><i
                                            class="fa-solid fa-pen-to-square"></i> Edit</a>


                                    <form class="delete-form" action="{{ route('offer.destroy', $blog->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger delete_offer mr-2" id=""
