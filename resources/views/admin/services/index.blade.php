    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <small class="text-muted float-end">
                <a href="{{ route('services.create') }}" class="btn btn-primary"><i class="fa-solid fa-plus"></i>
                    Create</a>
            </small>
        </div>
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($services as $key => $blog)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('services.edit', $blog->id) }}"
                                        style="float: left;margin-right: 5px;" class="btn btn-sm btn-primary"><i
                                            class="fa-solid fa-pen-to-square"></i> Edit</a>


                                    <form class="delete-form" action="{{ route('services.destroy', $blog->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger delete_services mr-2" id=""
